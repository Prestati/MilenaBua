...
export default function ProjectsAdmin({ initial }: { initial: Project[] }) {
  ...
  const save = async () => {
    setSaving(true);
    const res = await saveProjectsAction(items);
    setSaving(false);
    setMsg(res.success ? { ok: true, text: "Lagret!" } : { ok: false, text: res.error });
    setTimeout(() => setMsg(null), 3000);
  };

  const update = (slug: string, field: keyof Project, value: unknown) =>
          ...
          <div className="flex items-center justify-between px-5 py-3">
            <div className="flex items-center gap-3">
              ...
              <span className="text-[0.9rem] font-semibold" style={{ color: "var(--ink)" }}>
                {p.title || <em style={{ color: "var(--mid)" }}>Nytt prosjekt</em>}
              </span>
              {p.subtitle && (
                <span className="text-[0.7rem] font-medium" style={{ color: "var(--mid)" }}>{p.subtitle}</span>
              )}
            </div>
          ...
          {editingSlug === p.slug && (
            <div className="px-5 pb-5 pt-2 border-t grid sm:grid-cols-2 gap-4" style={{ borderColor: "var(--faint)" }}>
              ...
              <div className="sm:col-span-2">
                <label className="block text-[0.72rem] font-semibold mb-1" style={{ color: "var(--mid)" }}>Beskrivelse</label>
                <textarea
                  value={p.description}
